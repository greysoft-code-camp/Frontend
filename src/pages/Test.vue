     ...
     <q-list class="q-mt-md q-mb-xl">
        <q-item clickable v-ripple v-for="listItemName in listItems" :key="listItemName._id" v-show="listItemName.list === item" class="q-pa-xs">
          <q-item-section class="q-my-none q-px-md" style="background:rgb(128, 128, 128,0.2)">
             ...
             <draggable
              :list="listItems"
              group= listItemName.list
              @change="log"
              itemKey= listItemName
            >
              <template #item="{ element }" class="" >
                <div>
                <q-item-label style="text-transform:capitalize" v-show="listItemName.list === item"> {{element.value}} </q-item-label>
                </div>
              </template>
            </draggable>
...
export default {
  ...
  methods: {
    add: function() {
      this.list.push({ name: listItemName.body });
    },
    replace: function() {
      this.list = [{ name: listItemName.body }];
    },
    clone: function(el) {
      return {
        name: el.name + " cloned"
